    ...
    public function __construct($items, $rows, array $options = null)
    {
        ...
        for ($retries = 0; $retries < $MAX_RETRIES && (is_array($items) ? $candidateSpaces != 0 || $candidateAccuracy < 1 : $this->spaces != 0); $retries++) {
            ...
            */
                ...
                */
                ...
                for ($row = 0; $row < $ROWS && $remaining > 0; $row++) {
                    ...
                    if ($lastIsHigh) { // Cell is taken
                        ...
                        if (is_array($items)) {
                            if ($high) {
                                $currentItems[self::REQUIRED_HIGH]--;
                            } elseif ($large) {
                                $currentItems[self::REQUIRED_LARGE]--;
                            } else {
                                $currentItems[self::REQUIRED_SMALL]--;
                            }
                        }
            ...
            if (is_array($items)) {
                $this->accuracy = ($COUNT - abs($currentItems[self::REQUIRED_SMALL]) - abs($currentItems[self::REQUIRED_LARGE]) - abs($currentItems[self::REQUIRED_HIGH]) - 1.5 * $this->spaces) / $COUNT;
                //echo '$this->accuracy = ' . 100 * $this->accuracy . '%, $this->spaces = ' . $this->spaces . "\n";
                if ($this->accuracy > $candidateAccuracy) {
                    $candidate = $this->lines;
                    $candidateAccuracy = $this->accuracy;
        ...
        if (is_array($items)) {
            //echo '$this->accuracy = ' . 100 * $candidateAccuracy . "\n";
